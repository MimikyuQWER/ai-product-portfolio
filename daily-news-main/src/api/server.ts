...
export async function createServer(existingStore?: LocalStore) {
  ...
  fastify.addHook('preParsing', async (request, _reply, payload) => {
    if (!request.url.startsWith('/api/wave/callback')) {
      return payload;
    }
    ...
    (replay as any).receivedEncodedLength = Buffer.byteLength(rawBody);
    return replay;
  });
  ...
  fastify.addHook('preHandler', async (request, reply) => {
    ...
    const publicPaths = ['/api/login', '/api/ai/v1/register', '/api/ai/v1/verify', '/api/wave/callback', '/api/daily', '/api/plugins/metadata', '/api/analytics/track'];
  ...
  // --- Wave Bot Routes ---
  const { registerWaveRoutes } = await import('./waveRoutes.js');
  const { WavePublisher } = await import('../plugins/builtin/publishers/wave/WavePublisher.js');
  const wavePublisher = new WavePublisher(store);

  const envWaveConfig = {
    appId: process.env.WAVE_APP_ID || '',
    appSecret: process.env.WAVE_APP_SECRET || '',
    aesKey: process.env.WAVE_AES_KEY || '',
    signToken: process.env.WAVE_SIGN_TOKEN || '',
    apiBaseUrl: process.env.WAVE_API_BASE_URL || 'https://open.hoyowave.com',
    dailyViewBaseUrl: process.env.DAILY_VIEW_BASE_URL || '',
  };

  // 从 KV store 加载 Wave 配置，环境变量作为部署兜底，避免沙箱未写入 DB 时回调无法解密。
  const savedWaveConfig = await store.get('settings:WAVE_CONFIG');
  const savedConfig = savedWaveConfig ? (typeof savedWaveConfig === 'string' ? JSON.parse(savedWaveConfig) : savedWaveConfig) : {};
  const waveConfig = { ...envWaveConfig, ...savedConfig };
  if (waveConfig.appId) {
    wavePublisher.initialize(waveConfig);
  }
  registerWaveRoutes(fastify, wavePublisher);
  ...
  // API：保存 Wave 配置
  fastify.post('/api/wave/config', async (request, reply) => {
    const config = request.body as any;
    await store.put('settings:WAVE_CONFIG', JSON.stringify(config));
    wavePublisher.initialize(config);
    return { status: 'success', message: 'Wave configuration saved' };
  });

  fastify.get('/api/wave/config', async (request, reply) => {
    const saved = await store.get('settings:WAVE_CONFIG');
    const config = saved ? (typeof saved === 'string' ? JSON.parse(saved) : saved) : {};
    // 不返回敏感信息的完整值
    return {
      configured: !!config.appId,
      appId: config.appId || '',
      apiBaseUrl: config.apiBaseUrl || '',
      dailyViewBaseUrl: config.dailyViewBaseUrl || '',
    };
  });
  ...
  fastify.setNotFoundHandler((request, reply) => {
    ...
    } else if (request.url.startsWith('/daily/') || request.url.startsWith('/daily-assets/')) {
      reply.status(404).send({ error: `Static asset not found: ${request.url}` });
    } else {
