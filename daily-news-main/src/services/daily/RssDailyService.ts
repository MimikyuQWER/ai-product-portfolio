...
const groupTitles: Record<DailyRssSourceGroup, string> = {
  company: 'OpenAI / Google / Anthropic 公司动态',
  practitioner: '从业者观点',
  community: '社区热点',
  papers: '论文与模型',
  products: '每日产品',
...
const groupOrder: DailyRssSourceGroup[] = ['company', 'products', 'github', 'papers', 'practitioner', 'community'];
...
function parseSummaryObject(text: string): DailyRssSummary | null {
  if (!text) return null;
  // 去掉 markdown 代码块包裹
  let cleaned = removeMarkdownCodeBlock(text).trim();
  // 直接解析
  try {
    const obj = JSON.parse(cleaned);
    if (obj && typeof obj === 'object' && !Array.isArray(obj)) return obj as DailyRssSummary;
  } catch { /* fallthrough */ }
  // 用平衡括号提取第一个完整 JSON 对象（避免误匹配 tags 数组）
      ...
      if (cleaned[i] === '{') depth++;
        ...
        if (depth === 0) {
          try {
            ...
            if (obj && typeof obj === 'object') return obj as DailyRssSummary;
          } catch { /* ignore */ }
          break;
        }
      }
  ...
  async function summarizeOne(item: DailyRssItem): Promise<DailyRssItem> {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        const parsed = parseSummaryObject(response.content);
        if (parsed && parsed.summary_cn) return { ...item, summary: parsed };
      } catch {
        // 继续重试
      }
    }
