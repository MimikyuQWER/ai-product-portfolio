...
export class LocalStore {
  ...
  async init() {
    try {
      ...
      // 创建 Wave 订阅者表
      await this.db.exec(`
        CREATE TABLE IF NOT EXISTS wave_subscribers (
          id TEXT PRIMARY KEY,
          type TEXT NOT NULL,
          name TEXT DEFAULT '',
      ...
      `);
      await this.db.exec(`CREATE INDEX IF NOT EXISTS idx_page_views_date ON page_views(date)`);
      await this.db.exec(`CREATE INDEX IF NOT EXISTS idx_page_views_visitor ON page_views(date, visitor_id)`);
