...
import WaveBotModal from '../components/Wave/WaveBotModal';
...
function extractTitle(markdown?: string): string {
  return markdown?.match(/^#\s+(.+)$/m)?.[1]?.trim() || 'AI 资讯日报';
}
...
const Generation: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      fetch('/api/analytics/track', {
        ...
        body: JSON.stringify({ path: '/generation', visitorId }),
      }).catch(() => {});
    } catch {}
  }, []);
    ...
    if (!initialResult) {
      ...
      fetch(publicPath(`daily/${date}.md`))
        .then(async (res) => {
          ...
          if (contentType.includes('text/html') && text.includes('<div id="root"></div>')) {
            return null;
          }
          return text;
        })
        .then(text => {
          if (!alive) return;
          if (text) {
            setResult({ daily_summary_markdown: text });
            setStatus('草稿已生成（从 daily 文件加载）');
            return;
          }
  ...
  const markdown = result?.daily_summary_markdown || '';
  const title = useMemo(() => extractTitle(markdown), [markdown]);
  const tocItems = useMemo(() => extractToc(markdown), [markdown]);
  const availableDates = useMemo(() => {
    const dates = dailyDates.includes(date) ? dailyDates : [date, ...dailyDates];
    return [...new Set(dates)].sort().reverse();
  }, [dailyDates, date]);

  return (
    <div className="daily-editorial-page flex h-screen flex-col bg-white text-slate-900 dark:bg-background-dark dark:text-white">
      <div className="daily-editorial-topbar flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-border-dark px-4 sm:px-6 py-4">
        <div>
          <h1 className="text-slate-900 dark:text-white text-xl font-bold tracking-tight">AI 资讯日报</h1>
          <p className="text-slate-500 dark:text-text-secondary text-sm">按日期阅读、编辑和发布每日 AI 热点。</p>
        </div>
        <div className="daily-editorial-actions flex flex-wrap items-center gap-2 sm:gap-3 bg-white dark:bg-surface-dark p-1.5 rounded-lg border border-slate-200 dark:border-border-dark shadow-sm w-full sm:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 transition-all text-white text-sm font-medium px-3 py-1.5 shadow-md shadow-indigo-500/20"
            title="Wave 机器人订阅"
          >
            <span className="text-[16px]">🤖</span>
            <span className="hidden sm:inline">订阅推送</span>
          </button>
        </div>
      </div>

      <div className="daily-editorial-shell flex-1 grid grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)_280px] xl:grid-cols-[280px_minmax(0,1fr)_300px] min-h-0 overflow-hidden bg-white dark:bg-background-dark">
        ...
        <div className="daily-date-rail hidden lg:flex flex-col min-h-0 border-r border-slate-200 dark:border-border-dark bg-slate-50/80 dark:bg-surface-darker/50">
          <div className="px-4 py-4 border-b border-slate-200 dark:border-border-dark">
            <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-text-secondary mb-3">日报日期</div>
            <input
              className="w-full bg-white dark:bg-surface-dark text-slate-900 dark:text-white text-sm rounded-md border border-slate-200 dark:border-border-dark focus:ring-1 focus:ring-primary px-3 py-2 cursor-pointer"
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              onClick={(e) => (e.target as any).showPicker?.()}
          ...
          <nav className="flex-1 overflow-auto p-3 no-scrollbar">
            <div className="space-y-1">
              {availableDates.map((itemDate) => (
                <button
                  key={itemDate}
                  onClick={() => setDate(itemDate)}
                ...
                >
                  <div className="text-sm font-semibold">{formatDateLabel(itemDate)}</div>
                  <div className={`text-[11px] ${itemDate === date ? 'text-white/75' : 'text-slate-400 dark:text-text-secondary'}`}>{itemDate}</div>
                </button>
              ))}
            </div>
          </nav>
        ...
        <div className="daily-editorial-main flex flex-col min-w-0 min-h-0">
          <div className="daily-doc-toolbar flex items-center px-3 sm:px-4 py-2 h-12 border-b border-slate-200 dark:border-border-dark bg-slate-100 dark:bg-surface-darker shrink-0">
            <div className="flex items-center gap-1 text-slate-500 dark:text-text-secondary">
              <span className="material-symbols-outlined text-[18px] shrink-0 hidden sm:block">article</span>
              <span className="text-xs sm:text-sm font-medium whitespace-nowrap shrink-0">{title}</span>
            </div>
          </div>
              ...
              previewMode === 'preview' ? (
                <article className="mx-auto w-full max-w-[1100px]">
                  <ContentRenderer 
                    ...
                    className="daily-reader-markdown font-sans text-slate-700 dark:text-slate-300"
                  />
                </article>
              ) : (
        ...
        <aside className="daily-reader-toc hidden lg:flex flex-col min-h-0 border-l border-slate-200 dark:border-border-dark bg-slate-50/80 dark:bg-surface-darker/50">
          <div className="px-4 py-4 border-b border-slate-200 dark:border-border-dark">
            <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-text-secondary">目录</div>
          </div>
          <nav className="flex-1 overflow-auto p-4 no-scrollbar">
            {tocItems.length > 0 ? (
              <div className="space-y-1">
                {tocItems.map((item) => (
                  <a
                    ...
                    href={`#${item.id}`}
                    className={`block rounded-md py-1.5 text-sm leading-snug transition-colors hover:text-primary ${
                      item.level === 3
                        ? 'pl-5 text-slate-500 dark:text-text-secondary'
                        : 'pl-0 font-medium text-slate-700 dark:text-slate-300'
                    }`}
                  >
                    {item.text}
                  </a>
              ...
              <div className="text-sm text-slate-400 dark:text-text-secondary">暂无目录</div>
            )}
          </nav>
        </aside>
      </div>
      ...
      {/* Footer: 仅移动端日期切换 */}
      <div className="daily-editorial-footer flex lg:hidden items-center bg-white dark:bg-surface-darker px-4 py-3 border-t border-slate-200 dark:border-border-dark">
        <div className="flex w-full items-center gap-2 bg-slate-100 dark:bg-surface-dark rounded-lg p-2 border border-slate-200 dark:border-border-dark">
          ...
          >
            {availableDates.map((itemDate) => (
              <option key={itemDate} value={itemDate}>{formatDateLabel(itemDate)} · {itemDate}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      {/* Wave Bot Modal */}
      <WaveBotModal isOpen={showWaveModal} onClose={() => setShowWaveModal(false)} />
